...
description: 'Allow user to select which active process to view documents for'
...
nextStepFile: '{workflow_path}/steps/step-03-list-documents.md'
...
Allow the user to select which active process they want to view documents for.
...
- FORBIDDEN to scan documents in this step
...
- User needs to pick one process to view documents for
- This is a simple selection step
- Do not scan or load document information yet
...
Press Enter or type 1 to view documents for this process:
```
...
Display:
```
Select a process to view documents:
...
Load, read entire file, then execute `{nextStepFile}` (step-03-list-documents.md)
